...
def homepage(request):
    def GetMatchSummary(match):
        ...
        matchMVP = Player.objects.filter(Match__MatchID=match['MatchID'],
                                         Team="Team A")\
                                 .order_by('-ACS').values().first()

        MVPUsername = matchMVP['Username']
        MVPAgent = matchMVP['Agent']
        MVPAgentImage = AgentImage(MVPAgent)

        TagSplit = MVPUsername.split("#")
        MVPDisplayName = TagSplit[0]
        MVPUserTag = "#" + TagSplit[1]
        
        MVPKills = matchMVP['Kills']
        MVPDeaths = matchMVP['Deaths']
        MVPAssists = matchMVP['Assists']
        MVPKDA = "{}-{}-{}".format(MVPKills,MVPDeaths,MVPAssists)
        MVPKDR = MVPKills/MVPDeaths
        MVPACS = matchMVP['CombatScore']/matchMVP['RoundsPlayed']
        ...
        return {
            ...
            "MVP_DisplayName":MVPDisplayName,
            "MVP_UserTag":MVPUserTag,

            "MVP_Agent":MVPAgent,
            "MVP_AgentImage":MVPAgentImage,
    ...
    recentMatches = Match.objects.all().order_by("-Date")[:15]

    # Get form, record

    form = []
    for x in recentMatches.values_list("TeamOneWon","TeamOneLost","MatchDraw"):
        if x[0]:
            form.append("W")
        elif x[1]:
            form.append("L")
    ...
    wins = sum([x[0] for x in list(Match.objects.values_list("TeamOneWon"))])
    losses = sum([x[0] for x in list(Match.objects.values_list("TeamOneLost"))])
    draws = sum([x[0] for x in list(Match.objects.values_list("MatchDraw"))])
    record = "{}-{}-{}".format(wins,losses,draws)
    win_pct = (wins+0.5*draws)/(wins+losses+draws)
    num_matches = wins+losses+draws
    ...
    History = {
        ...
        "Record": record,
        "WinPct": win_pct,
        "WinPctStr": str(round(win_pct*100,2))+"%",
    ...
    Players = Player.objects.filter(Team="Team A").values("Username").annotate(
    ...
    ).filter(num_matches__gte=10).order_by('-num_matches')
    ...
    # Get last, random match

    FirstMatch = GetMatchSummary(Match.objects.order_by('-Date').values().last())
    RandomMatch = GetMatchSummary(Match.objects.order_by('?').values().first())
    LastMatch = GetMatchSummary(Match.objects.order_by('-Date').values().first())
